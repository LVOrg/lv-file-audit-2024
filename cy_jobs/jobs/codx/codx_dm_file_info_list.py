...
class CodxDMFileInfo(AppList):
    ...
    def get_items_by_app(self, app_name:str,recent:bool)->typing.Iterable[ObjectInfo]:
        sort = CodxRepository.dm_file_info.fields.CreatedOn.desc() if recent else CodxRepository.dm_file_info.fields.CreatedOn.asc()
        agg = CodxRepository.dm_file_info.tenant(app_name).context.aggregate().match(
            (self.filter_field==None)|(self.filter_field!=self.iter_version)
        ).sort(
            sort
        ).project(
            cy_docs.fields.object_id>>CodxRepository.dm_file_info.fields.id
        ...
        if len(items)>0:
            ...
            ret.DbContent = CodxRepository.dm_file_info.tenant(app_name).context
...
def main():
    a = CodxDMFileInfo(filter_field_name="lv-files.ocr", iter_version="03", app_name="developer")
