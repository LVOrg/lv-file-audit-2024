...
def run():
    consumer = Consumer(cyx.common.msg.MSG_FILE_EXTRACT_IMAGES_FROM_PDF)
    while True:
        time.sleep(1)
        msg = None
        try:
            ...
            if isinstance(msg, MesssageBlock):
                ...
                ic(url_resource)
                ret = remote_caller_service.get_image_from_pdf(
                    download_url=url_resource+"1",
                    upload_url =  url_upload_resource
                )
